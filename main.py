...
f”> `4.` Send the screenshot in the ticket channel\n”
...
_reset_done_today = False

@tasks.loop(minutes=1)
async def daily_reset_task():
global _reset_done_today
...
if local_now.minute < 5 and not _reset_done_today:
    _reset_done_today = True
...
async def close_ticket(self, interaction: discord.Interaction, button: ui.Button):
    if not interaction.user.guild_permissions.administrator:
        ...
        return
    await _close_ticket_channel(interaction.channel, interaction.guild, closed_by=interaction.user)
    try:
...
async def _close_ticket_channel(channel, guild, closed_by=None):
“”“Shared logic to close and delete a ticket channel.”””
uid_to_remove = None
for uid, ch_id in list(data[“open_tickets”].items()):
if ch_id == channel.id:
uid_to_remove = uid
break
if uid_to_remove:
...
```
closer_name = closed_by.display_name if closed_by else "System"
closing_embed = make_embed(
    "🔒 Ticket Closing",
    f"{Theme.SEP}\n\n"
    f"This ticket is being closed by **{closer_name}**.\n\n"
...
“”“Button shown on the payment embed to open a ticket.”””
def **init**(self):
super().**init**(timeout=None)
...
async def open_ticket(self, interaction: discord.Interaction, button: ui.Button):
    ...
    existing_ch_id = data["open_tickets"].get(uid)
    if existing_ch_id:
        existing_ch = guild.get_channel(existing_ch_id)
        if existing_ch:
            e = make_embed(
                "⚠️ Ticket Already Open",
                f"You already have an open ticket: {existing_ch.mention}\n\n"
                f"Please use your existing ticket or ask an admin to close it first.",
                Theme.WARNING
            )
    ...
    if not category:
        e = make_embed("❌ Configuration Error", "Ticket category not found. Contact an admin.", Theme.ERROR)
        await interaction.response.send_message(embed=e, ephemeral=True)
        return

    team_name = data.get("teams", {}).get(uid, {}).get("team", interaction.user.name)
    safe_name = "".join(c for c in team_name.lower() if c.isalnum() or c == " ").replace(" ", "-")
    ...
    try:
        ticket_channel = await guild.create_text_channel(
            ...
            reason=f"Payment ticket for {interaction.user}"
        )
    except discord.Forbidden:
        e = make_embed("❌ Permission Error", "Bot lacks permission to create channels. Contact an admin.", Theme.ERROR)
        await interaction.response.send_message(embed=e, ephemeral=True)
        return
    ...
    welcome_embed = make_embed(
        ...
        f"**Team:** `{team_name}`\n"
        f"**Amount Paid:** `₹{upi['payment_amount']}`\n"
        f"**UPI ID:** `{upi['upi_id']}`\n\n"
        f"{Theme.THIN_SEP}\n\n"
        f"**📸 Please send your payment screenshot here.**\n\n"
        f"An admin will verify and approve your slot shortly.\n\n"
        f"{Theme.THIN_SEP}\n\n"
        f"*Only you and admins can see this channel.*\n"
        f"*Admin will close this ticket after verification.*\n\n"
        f"{Theme.SEP}",
        Theme.ACCENT,
        "🎫 Ticket System"
    )
    ...
    confirm_embed = make_embed(
        "✅ Ticket Created",
        f"Your payment ticket has been opened: {ticket_channel.mention}\n\n"
        f"Please send your payment screenshot there.",
        Theme.SUCCESS
    )
...
# ================= 8. VERIFICATION SYSTEM (CONSENT) =================

class ConsentView(ui.View):
def **init**(self, team_name, leader, teammates):
timeout_minutes = data.get(“verify_timeout_minutes”, 5)
super().**init**(timeout=timeout_minutes * 60)
...
self.team_name = team_name
super().**init**(placeholder=“🎯 Select the 4 squad members…”, min_values=4, max_values=4)
...
super().**init**(label=label, style=style, disabled=(count >= MAX_SLOTS))
self.slot = slot
...
super().**init**(placeholder=“🔽 Select match to leave…”, min_values=1, max_values=1, options=options)
...
async def cancel_slot(self, interaction: discord.Interaction, button: discord.ui.Button):
    ...
    await interaction.response.send_message(embed=e, view=CancelView(booked), ephemeral=True)
...
e = make_embed(“❌ Invalid Match”, “Use: `MATCH_1`, `MATCH_2`, etc.”, Theme.ERROR)
...
e = make_embed(“❌ Not a Ticket”, “This command can only be used inside a ticket channel.”, Theme.ERROR)
await ctx.send(embed=e, delete_after=5)
return
await ctx.message.delete()
await _close_ticket_channel(ctx.channel, ctx.guild, closed_by=ctx.author)
...
“”“Approve payment — auto-adds to 4 matches based on selected part.”””
uid = str(member.id)
if uid not in data[“teams”]:
...
f”{Theme.SEP}\n\n**UPI ID:** `{upi_id}`\n**Name:** `{name}`\n**Amount:** `₹{amount}`\n\n{Theme.THIN_SEP}\n*These details will be shown to players during registration.*”,
Theme.SUCCESS
)
await ctx.send(embed=e)
...
f”{Theme.SEP}\n\n**UPI ID:** `{upi['upi_id']}`\n**Name:** `{upi['upi_name']}`\n**Amount:** `₹{upi['payment_amount']}`\n\n{Theme.THIN_SEP}\n*Use `!setupi <upi_id> <amount> <name>` to change.*”,
Theme.GOLD
)
await ctx.send(embed=e)
...
save_data(data)
e = make_embed(“✅ Timeout Updated”, f”{Theme.SEP}\n\nVerification consent timeout set to **{minutes} minutes**.”, Theme.SUCCESS)
await ctx.send(embed=e)
...
save_data(data)
e = make_embed(“✅ Part Renamed”, f”**Part {part_num}**: ~{old_name}~ → **{new_name}**”, Theme.SUCCESS)
await ctx.send(embed=e)
...
e = make_embed(“🔄 Part Status Updated”, f”**{part_name}** is now {status_str}”, color)
await ctx.send(embed=e)
...
open_tickets_count = len(data.get(“open_tickets”, {}))
embed = make_embed(
...
f”**Open Tickets:** `{open_tickets_count}`\n\n”
f”{Theme.THIN_SEP}”,
Theme.PREMIUM
)
for slot_name in SLOT_LIST_CHANNELS:
...
if is_admin:
options.extend([
discord.SelectOption(label=“Payment & Tickets”, description=“Approve, tickets, UPI settings”, emoji=“💰”, value=“payment”),
...
super().**init**(placeholder=“📖 Select a category…”, min_values=1, max_values=1, options=options)
...
def _overview(self):
    return make_embed(
        ...
        "> `5.` Click **🎫 Open Payment Ticket** and send screenshot\n"
...
def _player(self):
    return make_embed(
        ...
        "💡 *After paying, click **🎫 Open Payment Ticket** to submit your screenshot.*",
        Theme.TEAL
    )

def _payment(self):
    return make_embed(
        "💰 Payment & Ticket Commands",
        f"{Theme.SEP}\n\n"
        ...
        "**`!close`** (`!ct`) — *inside ticket channel*\n╰ Close and delete a ticket channel\n\n"
        "**`!add_to_ticket @user`** (`!att`) — *inside ticket channel*\n╰ Add another user to see the ticket\n\n"
        "**`!setupi <upi_id> <amount> <name>`** (`!supi`)\n╰ Set UPI payment details shown to players\n\n"
        "**`!viewupi`** (`!vupi`)\n╰ View current UPI settings\n\n"
        f"{Theme.THIN_SEP}\n"
        "💡 *Players register → select part → pay UPI → open ticket → admin approves*",
        Theme.GOLD
    )
